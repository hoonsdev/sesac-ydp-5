...
// 해시 함수의 한계: 레인보우 테이블
// -> 암호화된 비번을 빠르게 역추적해서 원본 비번을 알아낼 수 있음
console.log(createHashedPassword('hello'));
...
function saltAndHashPassword(pw) {
  ...
  // hash -> salt랑 pw를 결합해서 해시(암호화된 비번)를 생성
  const hash = crypto
...
// 2. 암호 비교 함수
// : inputPw(제공된 비번), salt, Hash 를 기반으로 새로운 해시를 생성하고
// savedHash와 비교
